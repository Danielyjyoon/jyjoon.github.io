...
class ToTensorTest_CT(object):
    def __call__(self, img):
        img = img.astype(np.float32)

        # Normalize and clamp between 0 and 1
        # print(img.max())
        # SHOULD CHANGE TRAINING AND INFERENCE CODE!!!! (fix 400)
        # img = np.clip(img / max(1., img.max()), a_min=0.0, a_max=1.0)
        img = np.clip(img / 400., a_min=0.0, a_max=1.0)
